...
class discordBot(commands.Bot):
    ...
    async def on_message(self,message):
        ...
        if message.author.id == '143004153910657024':
            ...
            if any(x in message.content for x in waifulist) or (len(message.content) == 1 and message.content == '婆'):
                if random.randint(0, 1) == 0:
                    await message.channel.send(file=discord.File('./assets/images/youmu_kick.jpg'),reference=message)
                else:
                    await message.channel.send(file=discord.File('./assets/images/mokou_kick.jpg'),reference=message)
                return
