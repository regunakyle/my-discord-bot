...
class Steam(commands.Cog):
    ...
    def checkGiveaway(self):
        ...
        for entry in rss.entries:
            if 'giveaway' in entry['title'] and 'expired' not in entry['summary']:
                #Time in UTC +0
                publishTime = util.strftime(datetime.datetime(*entry['published_parsed'][:6]))
                expiryDate = datePattern.search(entry['summary']).group(1) if datePattern.search(entry['summary']) else None
                try:
                    util.runSQL("Insert into steam_GiveawayHistory values (?,?,?,?)",(entry['title'],entry['link'],publishTime,expiryDate))
                    util.print('New record inserted into database. \nTitle: '+entry['title']+'\nPublish Time: '+publishTime+'\n')
                except:
    ...
    def getNewGiveaway(self,guildId):
        channel = util.runSQL('select BotChannel from guildInfo where GuildId = ?',(guildId,),True)[0]['BotChannel']
        if channel is None:
            return None,None
        ...
        """)
        filter = util.runSQL('select Keyword from steam_Blacklist where guildId = ?', (guildId,),True)
        filteredResults = []
        if results:
    ...
    async def getAllRecord(self,ctx):
        ...
        with open(location, 'w', newline='') as file:
            ...
            for item in util.runSQL("""select ltrim(Title,'[giveaway] ') 'Title',link,PublishTime,
                                        case when ExpiryDate is not NULL then ExpiryDate ELSE 'Unknown' END 'ExpiryDate'
                                        from steam_GiveawayHistory
                                        order by PublishTime desc""",None,True):
                writer.writerow([item['Title'],item['Link'],item['PublishTime'],item['ExpiryDate']])
    ...
    async def blacklist(self,ctx,domain:str = None,job:str = None):
        if job and domain and job.lower() == '-r':
            util.runSQL('delete from steam_Blacklist where Keyword = ?',(domain.lower(),))
            await ctx.send('If ***' + domain.lower() + '*** is in database, it should be deleted.',reference=ctx.message)
            await self.blacklist(ctx)
        else:
            if domain is None:
                blacklist = 'Keyword blacklist for ***' + ctx.guild.name + '***:\n'
                for item in util.runSQL('SELECT rowid,* FROM steam_Blacklist where GuildId = ?',(ctx.guild.id,),True):
                    blacklist += str(item['rowid']) + ':***' + item['Keyword'] + '***\n'
                blacklist = blacklist[:-1]
                await ctx.send(blacklist,reference=ctx.message)
            else:
                try:
                    util.runSQL('insert into steam_Blacklist values (?,?,datetime())',(domain.lower(),ctx.guild.id))
                    await ctx.send("Keyword ***"+domain.lower()+"*** added to database.",reference=ctx.message)
                except ValueError as e:
                    await ctx.send("Insert keyword failed: Probably because this keyword already exists in the database.",reference=ctx.message)
        ...
        for guild in self.bot.guilds:
            ...
            if newlist[1]:
                for item in newlist[1]:
                    ...
                    embed.add_field(name="Publish date", value=item['PublishTime'], inline=False)
                    if item['ExpiryDate']:
                        embed.add_field(name="Expiry date", value=item['ExpiryDate'], inline=False)
                    if channel:
